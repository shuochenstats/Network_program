mylocfdr<-function(zz, bre = 120, df = 7, pct = 0, pct0 = 1/4, nulltype = 1, 
          type = 0, plot = 1, mult, mlests, main = " ", sw = 0, all=1) 
{
  call = match.call()
  if (length(bre) > 1) {
    lo <- min(bre)
  ...
  if (missing(mlests)) {
    ...
    mlests = mylocmle(zz, xlim = c(med, b * sc),all=all)
    if (N > 5e+05) {
      warning("length(zz) > 500,000: For ML estimation, a wider interval than optimal was used.  To use the optimal interval, rerun with mlests = c(", 
              mlests[1], ", ", b * mlests[2], ").\n", sep = "")
      mlests = mylocmle(zz, xlim = c(med, sc),all=all)
    }
  }
  if (!is.na(mlests[1])) {
    if (N > 5e+05) 
    ...
    if (nulltype == 1) {
      ...
      ml.out = mylocmle(zz, xlim = c(mlests[1], b * mlests[2]), 
                      d = mlests[1], s = mlests[2], Cov.in = Cov.in,all=all)
      mlests = ml.out$mle
    }
    else mlests = mylocmle(zz, xlim = c(mlests[1], b * mlests[2]), 
                         d = mlests[1], s = mlests[2],all=all)
  ...
  Cov2.out = myloccov2(X, X0, i0, f, fp0["cmest", ], N)
  Cov0.out = myloccov2(X, matrix(1, length(x), 1), i0, f, fp0["thest", 
                                                            ], N)
  if (sw == 3) {
    if (nulltype == 0) 
